...
------------------------------------------------------------------------
-- Ersatz level generation.
------------------------------------------------------------------------

-- core.register_biome is overridden to remove biome dust definitions,
-- which must be placed manually after decorations are generated by
-- the level generator.

if mcl_levelgen.enable_ersatz then

local old_register_biome = core.register_biome
local old_unregister_biome = core.unregister_biome
local biome_dust_by_biome_id = {}

function core.register_biome (def)
	local cid_dust
	if def.node_dust then
		cid_dust = core.get_content_id (def.node_dust)
		def.node_dust = nil
	end
	local rc = old_register_biome (def)
	local id = core.get_biome_id (def.name)
	biome_dust_by_biome_id[id] = cid_dust
	return rc
end

function core.unregister_biome (name)
	local id = core.get_biome_id (name)
	biome_dust_by_biome_id[id] = nil
	old_unregister_biome (name)
end

core.register_on_mods_loaded (function ()
	core.ipc_set ("mcl_biomes:biome_dust_cids",
		      biome_dust_by_biome_id)
end)
...
local function register_biomes()
	...
	* Desert M
	...
	core.register_ore ({
		y_min = 1,
		y_max = mcl_vars.mg_overworld_max,
		ore = "mcl_powder_snow:powder_snow",
		-- ore = "mcl_core:glass_magenta",
		ore_type = "blob",
		biomes = {
			"Grove",
			"SnowySlopes",
...
local function register_decorations()
	...
	local common_spruce_schematics = {
		mod_mcl_core .. "/schematics/mcl_core_spruce_1.mts",
		mod_mcl_core .. "/schematics/mcl_core_spruce_2.mts",
		mod_mcl_core .. "/schematics/mcl_core_spruce_3.mts",
		mod_mcl_core .. "/schematics/mcl_core_spruce_4.mts",
		mod_mcl_core .. "/schematics/mcl_core_spruce_5.mts",
	}
	for _, spruce in ipairs (common_spruce_schematics) do
		core.register_decoration({
			deco_type = "schematic",
			place_on = {"mcl_core:snowblock"},
			sidelen = 80,
...
core.register_decoration({
	...
	for i=1,4 do
		core.register_decoration({
			...
			y_min = mcl_vars.mg_overworld_min,
			y_max = mcl_vars.mg_overworld_max,
			decoration = "mcl_flowers:pink_petals_"..i,
		})
		...
		core.register_decoration({
			...
			place_on = {"mcl_core:dirt_with_grass"},
			fill_ratio = 0.003,
			biomes = {"BirchForest","BirchForestM","ExtremeHills", "ExtremeHillsM", "ExtremeHills+"},
			y_min = mcl_vars.mg_overworld_min,
			y_max = mcl_vars.mg_overworld_max,
			decoration = "mcl_flowers:wildflowers_"..i,
		})
	end
